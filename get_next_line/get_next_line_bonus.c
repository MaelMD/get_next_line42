...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/11/11 11:42:10 by asabani           #+#    #+#             */
/*   Updated: 2021/11/11 12:12:31 by asabani          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	t_list		*head;
	static char	*buffer_remainder[65535];

	head = NULL;
	if (BUFFER_SIZE <= 0 || fd < 0)
		return (NULL);
	if (buffer_remainder[fd] && *buffer_remainder[fd])
		if (read_from_buffer(&head, buffer_remainder[fd], \
					buffer_remainder + fd))
			return (list_join_chunks(&head));
	read_from_file(fd, &head, buffer_remainder + fd);
	return (list_join_chunks(&head));
}
